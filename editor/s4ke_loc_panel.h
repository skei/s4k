#ifndef s4ke_loc_panel_included
#define s4ke_loc_panel_included
//----------------------------------------------------------------------
// 265*130
...
class s4ke_loc_panel : public Fl_Group
{
  public:
    Fl_Box*           label;
    ...
    static void selector_cb( Fl_Widget* o, void* p )
      {
        ...
        if ( sel->callback_context() == Fl_Table::CONTEXT_CELL ) par->do_callback(sel,(long)e_loc_select);
      }
    ...
    static void name_cb( Fl_Widget* o, void* p )
      {
        ...
        par->do_callback(inp,(long)e_loc_name);
      }

  public:

    s4ke_loc_panel(int X, int Y, int W, int H, const char* L=0)
    : Fl_Group(X,Y,W,H,L)
      {
        ...
        selector->end();
        selector->type = st_loc;

        label = new Fl_Box(X+130,Y,100,20,"Loc");
        ...
        //name->value( &s4ke_loc_name[ 0*256 ] );
